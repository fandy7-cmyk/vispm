...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (method === 'POST') {
      const { kode, nama, indeks, aktif } = body;
      ...
      await pool.query(
        'INSERT INTO master_puskesmas (kode_pkm, nama_puskesmas, indeks_beban_kerja, aktif) VALUES ($1, $2, $3, $4)',
        [kode, nama, parseFloat(indeks) || 0, aktif !== false]
      );
    ...
    if (method === 'PUT') {
      ...
      await pool.query(
        'UPDATE master_puskesmas SET nama_puskesmas=$1, indeks_beban_kerja=$2, aktif=$3 WHERE kode_pkm=$4',
        [nama, parseFloat(indeks) || 0, aktif !== false, kode]
      );
