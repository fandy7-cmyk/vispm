...
async function getUsulanList(pool, params) {
  ...
  const result = await pool.query(
    `SELECT uh.*, p.nama_puskesmas FROM usulan_header uh 
     LEFT JOIN master_puskesmas p ON uh.kode_pkm=p.kode_pkm ${ws} 
     ORDER BY uh.created_at DESC LIMIT 500`,
    qParams
  );
  ...
  const vpResult = await pool.query(
    `SELECT id_usulan, COUNT(*) as total, COUNT(CASE WHEN status='Selesai' THEN 1 END) as selesai 
     FROM verifikasi_program WHERE id_usulan=ANY($1) GROUP BY id_usulan`,
    [ids]
  );
  const vpMap = {};
...
async function getUsulanDetail(pool, idUsulan) {
  ...
  const result = await pool.query(
    `SELECT uh.*, p.nama_puskesmas FROM usulan_header uh 
     LEFT JOIN master_puskesmas p ON uh.kode_pkm=p.kode_pkm WHERE uh.id_usulan=$1`,
    [idUsulan]
  );
  if (result.rows.length === 0) return err('Usulan tidak ditemukan', 404);
  
  const vpResult = await pool.query(
    `SELECT email_program, nama_program, indikator_akses, status, catatan, verified_at 
     FROM verifikasi_program WHERE id_usulan=$1 ORDER BY created_at`,
    [idUsulan]
  );
...
async function saveDriveFolder(pool, body) {
  ...
  await pool.query(
    `UPDATE usulan_header SET drive_folder_id=$1, drive_folder_url=$2 WHERE id_usulan=$3`, 
    [driveFolderId, driveFolderUrl, idUsulan]
  );
...
async function buatUsulan(pool, body) {
  ...
  // Cek duplikat untuk operator yang sama
  const dupCheck = await pool.query(
    `SELECT id_usulan FROM usulan_header WHERE created_by=$1 AND tahun=$2 AND bulan=$3`, 
    [emailOperator, tahun, bulan]
  );
  ...
  // Ambil semua pengelola program aktif
  const ppResult = await pool.query(
    `SELECT email, nama, indikator_akses FROM users WHERE role='Pengelola Program' AND aktif=true`
  );
  ...
  try {
    ...
    // Insert header
    await client.query(
      `INSERT INTO usulan_header (id_usulan,tahun,bulan,periode_key,kode_pkm,total_nilai,total_bobot,
         indeks_kinerja_spm,indeks_beban_kerja,indeks_spm,status_kapus,status_program,status_final,
         status_global,is_locked,created_by,created_at)
       VALUES ($1,$2,$3,$4,$5,0,$6,0,$7,0,'Menunggu','Menunggu','Menunggu','Draft',false,$8,NOW())`,
      [idUsulan, tahun, bulan, `${tahun}-${String(bulan).padStart(2,'0')}-01`, kodePKM, totalBobot, indeksBeban, emailOperator]
    );
    ...
    // Insert verifikasi program untuk setiap pengelola program
    for (const pp of ppResult.rows) {
      await client.query(
        `INSERT INTO verifikasi_program (id_usulan,email_program,nama_program,indikator_akses,status,created_at) 
         VALUES ($1,$2,$3,$4,'Menunggu',NOW())`, 
        [idUsulan, pp.email, pp.nama, pp.indikator_akses||'']
      );
    }
...
async function hitungSPM(pool, idUsulan) {
  ...
  await pool.query(
    `UPDATE usulan_header SET total_nilai=$1, total_bobot=$2, indeks_kinerja_spm=$3, indeks_spm=$4 
     WHERE id_usulan=$5`,
...
// ============== APPROVE KAPUS (DENGAN CEK PKM) ==============
async function approveKapus(pool, body) {
  const { idUsulan, email, catatan } = body;
  
  // Ambil data usulan dan user
  const [usulanResult, userResult] = await Promise.all([
    pool.query('SELECT kode_pkm, status_global, status_kapus FROM usulan_header WHERE id_usulan=$1', [idUsulan]),
    pool.query('SELECT kode_pkm FROM users WHERE email=$1', [email])
  ]);
  
  if (usulanResult.rows.length === 0) return err('Usulan tidak ditemukan');
  if (userResult.rows.length === 0) return err('User tidak ditemukan');
  
  const usulan = usulanResult.rows[0];
  const user = userResult.rows[0];
  
  // CEK: Kapus hanya bisa verifikasi usulan dari PKM-nya sendiri
  if (user.kode_pkm !== usulan.kode_pkm) {
    return err('Anda hanya dapat memverifikasi usulan dari puskesmas Anda sendiri');
  }
  
  // CEK: Status harus Menunggu Kapus
  if (usulan.status_global !== 'Menunggu Kapus') {
    return err('Usulan tidak dalam status Menunggu Kapus');
  }
  
  // Setujui Kapus
  await pool.query(
    `UPDATE usulan_header SET status_kapus='Selesai', status_global='Menunggu Program',
     kapus_approved_by=$1, kapus_approved_at=NOW(), kapus_catatan=$2 
     WHERE id_usulan=$3`,
...
// ============== APPROVE PROGRAM (DENGAN CEK KAPUS SUDAH APPROVE) ==============
async function approveProgram(pool, body) {
  ...
  // CEK: Kapus harus sudah approve dulu
  if (usulan.status_kapus !== 'Selesai') {
    return err('Kepala Puskesmas harus menyetujui terlebih dahulu');
  }
  
  // CEK: Status harus Menunggu Program
  if (usulan.status_global !== 'Menunggu Program') {
    return err('Usulan tidak dalam status Menunggu Program');
  }
  
  // CEK apakah user ini terdaftar sebagai pengelola program
  const vpCheck = await pool.query(
    'SELECT id, status FROM verifikasi_program WHERE id_usulan=$1 AND email_program=$2', 
    [idUsulan, email]
  );
  if (vpCheck.rows.length === 0) {
    return err('Anda tidak terdaftar sebagai pengelola program untuk usulan ini');
  }
  if (vpCheck.rows[0].status === 'Selesai') {
    return err('Anda sudah memverifikasi usulan ini');
  }
  
  // Update verifikasi program
  await pool.query(
    `UPDATE verifikasi_program SET status='Selesai', catatan=$1, verified_at=NOW() 
  ...
  // Cek apakah semua pengelola program sudah approve
...
// ============== APPROVE ADMIN (DENGAN CEK KAPUS & PROGRAM SUDAH APPROVE) ==============
async function approveAdmin(pool, body) {
  ...
  // CEK: Kapus harus sudah approve
  if (usulan.status_kapus !== 'Selesai') {
    return err('Kepala Puskesmas harus menyetujui terlebih dahulu');
  }
  
  // CEK: Program harus sudah approve (semua pengelola program)
  if (usulan.status_program !== 'Selesai') {
    return err('Semua Pengelola Program harus menyelesaikan verifikasi terlebih dahulu');
  }
  
  // CEK: Status harus Menunggu Admin
  if (usulan.status_global !== 'Menunggu Admin') {
    return err('Usulan belum siap untuk diverifikasi Admin');
  }
  
  // Approve final
  await pool.query(
    `UPDATE usulan_header SET status_final='Selesai', status_global='Selesai', is_locked=true,
     admin_approved_by=$1, admin_approved_at=NOW(), admin_catatan=$2,
     final_approved_by=$1, final_approved_at=NOW() 
     WHERE id_usulan=$3`,
    [email, catatan||'', idUsulan]
  );
