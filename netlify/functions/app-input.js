...
function _showPDFFallback(container, errMsg, btnDl) {
  // Coba tampilkan via Google Docs Viewer sebagai fallback utama
  const _f = window._modalBukti && window._modalBukti.links && window._modalBukti.links[window._modalBukti.idx || 0];
  const _rawUrl = _f ? (_f.url || '').split('?')[0] : null;
  // Pastikan URL berakhiran .pdf agar Google Docs Viewer mengenali format
  const _pdfUrl = _rawUrl && !_rawUrl.toLowerCase().endsWith('.pdf') ? _rawUrl + '.pdf' : _rawUrl;
  const googleUrl = _pdfUrl ? `https://docs.google.com/viewer?url=${encodeURIComponent(_pdfUrl)}&embedded=true` : null;

  const SVG_PDF  = `<svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>`;
  ...
  if (googleUrl) {
    // Tampilkan Google Docs Viewer sebagai iframe — tidak butuh proxy
    const _gdId = 'gdFallback_' + Date.now();
    const _gdLoadId = 'gdLoad_' + Date.now();
    container.innerHTML = `
      <div style="width:100%;height:100%;display:flex;flex-direction:column;">
        <div style="flex:1;position:relative;min-height:0;">
          <iframe id="${_gdId}" src="${googleUrl}"
            style="width:100%;height:100%;border:none;opacity:0;transition:opacity 0.4s"
            onload="this.style.opacity=1;var m=document.getElementById('${_gdLoadId}');if(m)m.style.display='none';"
          ></iframe>
          <div id="${_gdLoadId}" style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;color:#94a3b8;pointer-events:none">
            ${SVG_PDF}
            <div style="font-size:13px;font-weight:600;color:white">Memuat via Google Docs...</div>
            ${errMsg ? `<div style="color:#64748b;font-size:11px;max-width:260px">${errMsg}</div>` : ''}
            <div style="width:28px;height:28px;border-radius:50%;border:3px solid transparent;border-top-color:#0d9488;animation:spin 1s linear infinite;margin-top:4px"></div>
          </div>
        </div>
        <div style="flex-shrink:0;padding:10px 16px;border-top:1px solid rgba(255,255,255,0.08);background:#1e293b;display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;">
          <span style="font-size:11px;color:#64748b;display:flex;align-items:center;gap:5px">${SVG_INFO} Pratinjau via Google Docs. Jika tidak muncul, silakan download.</span>
          <div style="display:flex;gap:8px;">
            <a href="${_pdfUrl}" target="_blank" style="background:#334155;color:white;padding:7px 14px;border-radius:7px;font-size:12px;font-weight:600;text-decoration:none;display:inline-flex;align-items:center;gap:5px">Buka Tab Baru</a>
            ${btnDl}
          </div>
        </div>
      </div>`;
  } else {
    // Tidak ada URL sama sekali — tampilkan pesan error biasa
    container.innerHTML = `
