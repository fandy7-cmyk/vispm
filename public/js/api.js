...
function showConfirm({ title, message, onConfirm, type = 'danger' }) {
  ...
  header.className = 'confirm-header' + (type === 'warning' ? ' warning' : '');
  const icon = document.getElementById('confirmIcon');
  icon.textContent = type === 'warning' ? 'warning' : 'delete';

  const btn = document.getElementById('confirmActionBtn');
  btn.className = 'btn-confirm-delete' + (type === 'warning' ? ' btn-confirm-warning' : '');
