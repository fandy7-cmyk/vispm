...
async function findOrCreateFolder(token, name, parentId) {
  ...
  // supportsAllDrives=true agar bisa akses folder milik orang lain yang di-share
...
async function uploadFile(token, fileName, mimeType, base64Data, folderId) {
  ...
  catch(e) { throw new Error('Response non-JSON (' + uploadRes.status + '): ' + text.substring(0, 300)); }

  if (!data.id) throw new Error('Upload gagal (' + uploadRes.status + '): ' + JSON.stringify(data));
