...
function mapHeader(r) {
  ...
  if (isDitolak) {
    ...
    } else if ((r.ditolak_oleh||'') === 'Admin' || (r.status_program||'') !== 'Ditolak') {
      ditolakOleh = 'Admin';
...
async function restoreVerifStatus(pool, body) {
  ...
  await pool.query(
    `UPDATE usulan_header SET status_kapus='Selesai', status_program='Selesai',
     ...
     WHERE id_usulan=$1`, [idUsulan, kapusBy||'restored', kapusAt||new Date().toISOString()]
  );
  await pool.query(
    `UPDATE verifikasi_program SET status='Selesai', verified_at=COALESCE(verified_at,NOW()),
...
async function getLogAktivitas(pool, idUsulan) {
  if (!idUsulan) return err('idUsulan diperlukan');
  const r = await pool.query(
    `SELECT * FROM log_aktivitas WHERE id_usulan=$1 ORDER BY timestamp DESC`, [idUsulan]
  );
  return ok(r.rows);
}
